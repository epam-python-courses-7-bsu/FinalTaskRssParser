...
def converting_to_html(path, pack_of_news):
    ...
    with open(path_to_file, 'w', encoding='utf-8') as html_news:
        for item in pack_of_news:
            ...
            directory = checking_path(path)
            file_name = path + directory + name_by_date + '.jpg'
            html_news.write("<p>{0}.<br> <strong>Title: {1}</strong><br>".format(item.number_of_novelty,
                                                                                 item.title_of_novelty))
            html_news.write("<span>Date: {0} </span><br> Link: <a href="">{1}</a><br>".format(item.time_of_novelty,
                                                                                              item.source_link))
            html_news.write("<strong>Description</strong>:<br>{0}<br>".format(item.description))
            try:
                html_news.write("Image: <br><img src='{0}' title='{1}'>".format(file_name, item.alt_text))
                html_news.write("<br>Image links: <a href="">{0}</a> <br>Alternative text: {1}<br>".
                                format(item.images_links,
                                       item.alt_text))
            except RuntimeError:
                html_news.write("Image links: {0}<br> Alternative text: {1}<br>".format(item.images_links,
                                                                                        item.alt_text))
